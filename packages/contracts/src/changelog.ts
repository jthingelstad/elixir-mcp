...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    date: "2026-09-19",
    summary:
      "One grammar, one vocabulary (docs/reviews/2026-09-19-INTERFACE-REVIEW.md Part 2, Phase 4). Every windowed tool's applied.window says its season: season (the season the window starts in; null on an unbounded window), crosses (every season roll inside it, empty when clean) and season_age_days, on battles_query, battles_performance, battles_decks, battles_cards, battles_opponents, battles_compare, clans_standings, battles_levels, clans_pilot_scores, clans_participation, rankings_timeline, game_events and elixir_timeline as the meta and series tools already carried them; the crossing note fires only when crosses is non-empty. The player battle tools and clans_standings take season ('current', 'previous', 2026-08 or 135), so 'this season' is one argument on every tool that has a window; their default is unchanged. The series tools (players_timeline, clans_timeline, clans_members_timeline) accept an instant for from/to and floor it to its game day (10:00Z grid), echoing the instant under applied.window.floored with a note, where they refused it; days: N is N game days, today included. The one point vocabulary: players_timeline points carry day beside date; rankings_timeline points carry day (the snapshot's game day); battles_levels monthly_trend marks the window's clipped months with partial: true and covers {from, to}, the weekly shape; clans_participation echoes applied.window.source 'default' when weeks was defaulted and its notes say war_points is the period points figure; game_events carries game_days_seen beside days_seen (the same sightings on the game day grid; days_seen retires at 4.0.0), its running_on_latest_day is a fact of the table rather than of the window, and a date-only to no longer reaches one day past itself; clans_roster.lifetime carries profile_observed_at beside as_of (as_of retires at 4.0.0). clans_pilot_scores and battles_levels monthly_trend take mean_starting_trophies over ladder battles only (a Path of Legends figure is a league rating on another scale; null with none), and say so. players_summary top_deck and best_deck carry mean_level_gap, so the comparability note names real gaps. Descriptions: players_timeline.metrics names its four groups and the default; battles_performance.group_by says mode is the named game mode, not the group; clans_members_timeline.limit says the order and how to choose; rankings_players says rating is the profile's pol_trophies, verified equal on the live API. Additive; no migration.",
  },
  {
    version: "3.16.0",
    date: "2026-09-19",
  ...
  },
  {
    ...
    date: "2026-09-19",
    summary:
      "The record reaches the wire (docs/reviews/2026-09-19-INTERFACE-REVIEW.md Part 1.3, Phase 2): every collected column an agent would ask about is served on the tool that owns the question, no migration. battles_query rows carry mode_group (the contract's fold of type) and, at full verbosity, context {event_tag, tournament_tag, ladder_tournament, hosted, deck_selection} and on a boatBattle row boat {side, towers_before, towers_after, remaining}; compact carries deck_selection at the top level. war_history with season_id and section_index carries days[] (the race's own day-by-day from the API's periodLogs: per closed war day every clan's points_earned, progress_start, progress_end, progress_earned, end_of_day_rank, defenses_remaining, progress_from_defenses) and standings[] (every clan in the bracket with clan_score and repair_points); weeks[] carry closed_at (the API's own close instant) beside finished, plus our_clan_score and our_repair_points; member_weeks[] carry repair_points. war_current carries days_closed[] in the same shape (full verbosity), clan_score and repair_points on standings[], repair_points per participant, and period.api_period_type (the API's own word for the day). war_rivals rows carry clan_score, latest observed. players_profile.snapshot.path_of_legend.seasons[] lists the last twelve season finals the record kept ({season_month, league, trophies, rank}, newest first); players_profile.attributes and clans_roster.lifetime carry war_day_wins, clan_cards_collected and legacy_trophy_road_high_score. clans_roster carries the clan's type, location_id and description at both verbosities; clans_timeline's metrics enum gains type and location_id (asked for, never default). Additive.",
  },
  {
    version: "3.14.0",
    date: "2026-09-19",
