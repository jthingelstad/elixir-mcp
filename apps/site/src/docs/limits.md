...
| Query budget | the analytical reads (`battles_meta_decks`, `battles_meta_cards`, `clans_standings`) | per call | 18 seconds of database time, shortened when the function has less time left | tool error `query_timeout` with a retry hint and `meta.request_id`; no partial aggregation (see [Protocol](/docs/protocol#errors)) |
