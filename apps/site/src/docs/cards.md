...
| `members` | on a clan segment: `played` (each member's battles with it, win rate, `level_played`, forms) and `held` (each member's level, forms unlocked, star level from the collection snapshot), with how many members have a recorded collection | clan segments |
