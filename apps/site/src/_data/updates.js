...
export default [
  ...
  {
    date: "2026-09-23",
    title: "The timeline pages cleanly and reads past weeks as they were",
    body: "The Elixir Gym's second run over the timeline. When a long window is split across pages, continuing from where the last one stopped no longer loses a moment at the split or repeats one. Every standout session is listed, not just the first five. And a look back at a past week now shows that week's war and donations, not today's. Contract 6.34.0.",
  },
  {
    date: "2026-09-23",
  ...
  {
    date: "2026-09-22",
    title: "Card of the Week: a seventh email, and a page for every card",
    body: "Friday now has an issue, and it is about one card. The card is chosen by a program rather than by us - the ten most-played cards of the season that have not been written up in the last year, with one drawn from those ten - so nothing gets featured because somebody liked it, and nothing gets featured twice in a year. A card is used up by an issue that actually sends: an issue that fails its checks leaves that card still due its turn. It is written the way the Top 100 is written. A program builds the brief from the record (the closed week's usage and win rate, where the card stands among all cards this season, every recorded season as a series, the split by mode and by trophy band, what the global Path of Legends top 100 do with it, what it travels with, and the decks carrying it); the writer may print a number only after reading it from the brief through a tool; an editor pass corrects the draft; and before it sends, every number in it is checked back against the brief. An issue that fails does not send. Two things we would rather say out loud: the headline covers the game week that closed on Monday while modes, partners and decks cover the season so far, because the record can only answer those over a whole season, and the footer of every issue says so - and the deck blocks are printed by the mail from the record, never typed by the model, because two different decks can share one archetype name and a deck list written from a label would be the wrong deck. Every card also has a page now, at /cards/<id>, with no sign-in: the catalog row, the season series, the split by mode, and the issue about it if there has been one. It is where a forwarded email lands. Switch it off like any other kind on Profile - Email.",
  },
  {
    date: "2026-09-22",
    title: "Events are their own thing now, and they stop skewing the meta",
    body: "Clash Royale game modes are played as different games. A river race battle only somewhat resembles Trophy Road, and neither tells you much about Path of Legends - and in some modes you do not even pick your deck. Elixir had one blunt fold for this, keyed on the battle's type, and it was quietly wrong: the API files everything from a two-week 2v2 tournament to the reworked Seasonal Trophy Road under one type, 'trail', which Elixir called 'casual'. So the Seasonal Road - a real ladder, but one that bans your best eight cards and floors the rest at level 15 - was being counted as casual play, and its level-16 decks were informing what Elixir told you about decks generally. The fix is the API's own marker: a battle played inside a time-bound event carries an eventTag, and nothing else does - 100% of those 123,562 battles against 0% of every permanent format. Event content is now its own mode group, mode: 'event' selects it, every other mode excludes it, and the meta ignores it entirely along with any deck you did not choose yourself. It is all still there to read battle by battle; it just no longer speaks for the game. Contract 6.17.0.",
  },
  {
    date: "2026-09-22",
    title: "A duel can finally tell you how round two went",
    body: "A river race duel is up to three games recorded as one row, and Elixir has been telling people it could not speak to the games inside it - that a duel's tower hitpoints describe the final round only, that it has no elixir differential. That was true of what Elixir kept, not of what the game reports: the API sends every round separately, with its own crowns, tower hitpoints and elixir leaked, and Elixir stored each round's DECK while throwing the round's RESULT away. It keeps them now, and 20,218 rounds across 4,362 duels were filled back from the payload archive, so the history answers too. A duel row carries rounds[] beside its decks, on the same round numbers, each with its own elixir differential - the comparison the summed counter can never give you. Also in this version: global_rank on every participant, the global leaderboard position the API reported on that battle, so 'I was up against someone ranked 87th' is a thing the record can say. Contract 6.16.0, additive.",
  },
  {
    date: "2026-09-22",
    title:
  ...
  },
  {
    date: "2026-09-20",
    title:
      "Meta decks against your own collection: what you can field, and what a few upgrades would open",
    body: "An agent recommended a deck from the corpus meta that the player could not play: two of the top four rows ran a card he does not own, and the top row, at his actual card levels, sat two mean levels below what he had been fielding all month. The population's decks are facts about the population; sorted by win rate they read as advice, and nothing in the response checked what the caller holds. Now battles_meta_decks and battles_meta_cards take fit_for, a player tag: rows the player cannot field (a card not owned, a form not unlocked) leave the deck list for an unfieldable list that says why, every remaining row says what the deck would be at the player's levels against what they have actually been fielding, and lists the upgrade path - each card below that level and how far it has to go - so an agent can say both 'here is what you can play today' and 'this deck opens up if Archer Queen and Earthquake come up five levels'. players_collection carries the same benchmark: the mean level of the decks you have really played in the last thirty days. Contract 6.4.0, additive.",
  },
  {
    date: "2026-09-20",
    title:
      "A tag never stands alone: every clan and player on the console is named",
    body: "A pass over the console for places that showed a clan tag or a player tag with no name beside it. Agents: the clan an agent acts for read as its tag on the agent's page, in the agents table and on Connections, and the create-agent form's clan picker promised a name it never had; the name now travels with the tag everywhere a principal's clans are listed. Explore: a search result's clan, a battle record's own player, a war week's clan, and the titles of a player's battles, decks and war-weeks lists all read as tags; the envelopes behind them now carry the last-observed name (battles_query, battles_decks, war_history, players_search; contract 6.3.0, additive) and the pages show it. Smaller: the Explore 'try' chip for your own player reads your name (or nickname) instead of your tag, an account event about a player or clan is named from the record, and the admin usage table names each account's primary player. Same rule on Elixir Clan: who decided a card, who set a hold, who granted an award and who saved a policy version were stamped as tags alone and now carry the name (rows written before today are named from what the ledger already knows), the public How-Elder-works page names the clan, and feedback carries the clan's name beside its tag.",
  },
  {
    date: "2026-09-20",
