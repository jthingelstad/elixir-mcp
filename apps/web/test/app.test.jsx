...
test('landing renders the pitch, the disclaimer, and posts request-access', async () => {
  ...
  render(<App />);
  expect(await screen.findByText(/Clash Royale history, recorded/)).toBeTruthy();
  expect(screen.getByText(DISCLAIMER)).toBeTruthy();

  fireEvent.change(screen.getByLabelText(/Email/), { target: { value: 'a@b.com' } });
  fireEvent.change(screen.getByLabelText(/player tag/i), { target: { value: '#20JJJ2CCRU' } });
  fireEvent.click(screen.getByRole('button', { name: 'Request access' }));
  await waitFor(() => expect(screen.getByText(/If your request is approved/)).toBeTruthy());
...
test('sign-in flow: email step then code step authenticates', async () => {
  global.fetch = mockFetch({
    ...
    'POST /api/auth/code': (init) => {
      expect(JSON.parse(init.body)).toEqual({ email: 'j@x.com', code: '123456' });
      return [200, { authenticated: true }];
    },
  });
  render(<App />);
  fireEvent.click(await screen.findByText('Sign in'));
  fireEvent.change(await screen.findByLabelText(/Email/), { target: { value: 'j@x.com' } });
  ...
  await waitFor(() => expect(global.fetch).toHaveBeenCalledWith('/api/auth/code', expect.anything()));
});
  ...
  global.fetch = mockFetch({
    'GET /api/me': [
      200,
      {
        ...
        claims: [
          { player_tag: '#20JJJ2CCRU', status: 'verified', is_primary: true, name: 'Jamie', last_known_clan_tag: '#J2RGCRVG' },
        ],
        recordings: [{ subject_tag: '#20JJJ2CCRU', status: 'active', freshest_poll: new Date().toISOString() }],
      },
    ],
  });
  render(<App />);
...
test('admin view is owner-gated in the UI', async () => {
  ...
  global.fetch = mockFetch({
    'GET /api/me': [200, { authenticated: true, is_owner: false, claims: [], recordings: [] }],
  });
  render(<App />);
