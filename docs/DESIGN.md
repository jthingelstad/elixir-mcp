...
- **Two hostnames vs one path-split host:** one CloudFront with a no-cookie behavior on the MCP paths would work, but Jamie chose the two-name layout (`elixir.` site, `mcp.` protocol door) — reads better, mirrors librarian exactly, costs one DNS record.
