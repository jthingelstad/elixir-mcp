> **ARCHIVED 2026-09-08. This is the v2 design as written on 2026-09-03 and it
> is no longer accurate.** It predates the three-principal model (users, agents,
> integrations), per-principal resource URLs, relationships, and a dozen
> contract versions. It is kept because the REASONING is still worth reading —
> why canonical battle ids, why one global rate budget, why entitlements rather
> than permissions.
>
> For what is true now: the [public documentation](https://elixir.poapkings.com/docs)
> is the product's source of truth, and `docs/NOTES.md` is the decision log.
> `docs/ENGINEERING.md` keeps the invariants that were never public.
