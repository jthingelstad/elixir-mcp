...
- The new catalogue-only read showed `recording#participation-by-week` correctly documents `war_decks_by_day` and `war_battles_by_day`, but the only observed full participation reads were eight-week ones that correctly hit the agent cap. A bounded two-week full seed now makes the promised fields observable to the docs check, with a regression test. The same single run had two marginal timing-budget overruns (`badges_rarity` 4,064 ms over 4,000; `battles_meta_decks` 10,950 ms over 9,974); both had passed in the earlier full run and production remained healthy, so they are a watch rather than a re-run or threshold change.
