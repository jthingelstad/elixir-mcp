...
- Jamie asked whether a skill like elixir-bot's `cr-api-doc-audit` belongs here or in the reference, reading Elixir's S3 archive. Here: the reference is public and caller-neutral (no consumer notes, no raw tags), and the archive, its profile and the manifest's path grammar are the hub's. The archive is the better evidence: every distinct payload since recording began (about 240,000, 1.7 GB), every recorded clan and player, and fifteen endpoints including boards and event lists a single-clan tool never reads; the bot's buffer is one clan for 60 days.
