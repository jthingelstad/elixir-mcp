...
- **DEPLOYS BLOCKED (2026-09-24 21:19Z).** The deploy of 7fb2e85 failed on EmailRelayFunction ("Could not find a value associated with JSONKey in SecretString") and rolled back cleanly; production runs the 6659f7a build (SES, Buttondown enrollment working from its resolved env). The new relay resolves only `buttondown_api_token`, so that key left `elixir-mcp/app` in the 21:16Z secret edit along with `jmap_token`. The previous template needs `jmap_token` too, so no revision deploys until the secret has `buttondown_api_token` again (Jamie: console, or `AWS_PROFILE=cloud-engineer node infra/scripts/secret-add-keys.mjs buttondown_api_token=BUTTONDOWN_API_TOKEN`, which copies it from .env unseen) or Buttondown enrollment is retired. Then redeploy main.
