...
- **War facts are weekly aggregates on every surface** — the API does not say which day a war deck was played and each race rolls at its own moment, which one clan can calibrate but Elixir cannot place reliably at its scale; so no MCP tool or JSON API operation splits a member's week by war day: no per-day attendance (`war_days_battled`, `war_days`, `attendance_by_war_day`, the participation day arrays), no training-day total, no after-the-finish `scoring_decks`. Served: the game's weekly counters, the race's own day-by-day (`days`, `finish_war_day`) and `war_current.decks_today`, the game's count for the day in progress. Supersedes "war-day attribution stays the informational 10:00Z grid" above. (2026-09-25; Jamie)
