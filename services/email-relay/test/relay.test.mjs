  ...
  const fetchImpl = async (url, init = {}) => {
    ...
    const responses = body.methodCalls.map(([name, , tag]) => {
      if (name === 'Identity/get')
        return [name, { list: [{ id: 'id1', email: 'elixir@poapkings.com' }] }, tag];
      if (name === 'Mailbox/query') return [name, { ids: ['drafts1'] }, tag];
      if (name === 'Email/set') return [name, { created: { draft: { id: 'e1' } } }, tag];
      if (name === 'EmailSubmission/set') return [name, { created: { send: { id: 's1' } } }, tag];
      return [name, {}, tag];
    });
...
test('JMAP sender: session -> identity/mailbox -> Email/set + EmailSubmission/set', async () => {
  ...
  const send = makeJmapSender({ token: 't', fromEmail: 'elixir@poapkings.com', fetchImpl });
  ...
  const submission = sendCall.methodCalls.find(([n]) => n === 'EmailSubmission/set')[1];
