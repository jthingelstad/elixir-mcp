...
export function makeJmapSender({ token, fromEmail, fetchImpl = fetch }) {
  ...
  async function bootstrap() {
    ...
    const res = await fetchImpl(SESSION_URL, {
  ...
  return async function send({ to, subject, text }) {
    ...
    if (!created) {
      const err = responses.find(
        ([name]) => name === "EmailSubmission/set",
      )?.[1]?.notCreated;
      throw new Error(
        `JMAP submission failed: ${JSON.stringify(err ?? responses)}`,
      );
    }
    return { sent: true };
  };
