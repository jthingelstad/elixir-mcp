...
export function renderEmail(msg) {
  ...
  if (msg.kind === "welcome") {
    return {
      ...
      text:
        `You're in!\n\n` +
        `Sign in at ${SIGNIN_BASE}, then add your player — adding is recording, there is no\n` +
        `separate step. Connect your agent to ${SITE}/mcp and start asking\n` +
      ...
      html: shell({
        ...
        body: [
          ...
          p(
            "Sign in, then add your player &mdash; adding <em>is</em> recording, there is no separate step to remember. Then connect your agent and ask it something the game itself can&rsquo;t answer.",
          ),
