...
export async function loadMigrations(dir) {
  ...
  for (const name of entries) {
    ...
    if (!m) throw new Error(`migration file name not NNNN_snake_case.sql: ${name}`);
    const id = Number(m[1]);
    if (id !== expected) {
      throw new Error(`migration ids must be dense and ordered; expected ${expected}, got ${name}`);
    }
    expected += 1;
    const sql = await readFile(path.join(dir, name), 'utf8');
    migrations.push({ id, name, sql, sha256: createHash('sha256').update(sql).digest('hex') });
  }
  return migrations;
}
