...
/** {event_payload_census: {limit?: 5000}} - read-only: renders every
 *  event's facts from its typed columns (0124, mcp/event-payloads.mjs)
 *  and compares them with the JSON still stored beside them, so the
 *  drop of the JSON is decided on the live record, not the fixtures.
 *  Rival lists compare order-free; a `recorded` flag that moved since
 *  the observation is reported apart from other differences. */
export async function eventPayloadCensus(databaseUrl, spec = {}) {
  const {
    hydratePlayerEvents,
    hydrateClanEvents,
    PLAYER_EVENT_COLUMNS,
    CLAN_EVENT_COLUMNS,
  } = await import("../../mcp/src/event-payloads.mjs");
  const limit = Math.min(50000, Math.max(100, Number(spec.limit ?? 5000)));
  const db = new pg.Client({ connectionString: databaseUrl });
  await db.connect();
  const started = Date.now();
  const canon = (v) =>
  ...
  const rivalsSorted = (p) =>
    p && Array.isArray(p.rivals)
      ? {
          ...p,
          rivals: [...p.rivals]
            .map(({ recorded: _recorded, ...r }) => r)
            .sort((a, b) => (a.tag < b.tag ? -1 : 1)),
        }
      : p;
  try {
    const out = { limit, ms: 0, player: {}, clan: {}, examples: [] };
    for (const [stream, cols, hydrate] of [
      ["player", PLAYER_EVENT_COLUMNS, hydratePlayerEvents],
      ["clan", CLAN_EVENT_COLUMNS, hydrateClanEvents],
    ]) {
      let after = 0;
      for (;;) {
        const { rows } = await db.query(
          `select ${cols}, payload as stored from ${stream}_event
        ...
        for (const r of rows) {
          const tally = (out[stream][r.event_type] ??= {
            rows: 0,
            same: 0,
            differ: 0,
          });
          tally.rows += 1;
          const a = canon(rivalsSorted(r.payload));
          const b = canon(rivalsSorted(r.stored));
          if (a === b) tally.same += 1;
          else {
            tally.differ += 1;
            if (out.examples.length < 12)
              out.examples.push({
                stream,
                event_id: r.event_id,
                kind: r.event_type,
