...
before(async () => {
  ...
  await ingestClanRoster(db, { payload: roster, observedAt: '2026-09-03T14:40:34Z' });
  ...
  invoke = makeInvoker({
    db,
    account: { accountId: alice.account_id, isOwner: false, timezone: 'America/Chicago' },
    registry,
  });
  const {
    rows: [out],
...
test('get_clan: roster with roles, snapshots slots, membership tenure', async () => {
  ...
  assert.ok(body.members.every((m) => m.member_since_observed), 'observed tenure on every member');
});
...
test('get_war_history: ranks per week and one member focus with attendance', async () => {
  ...
  const focused = await call(invoke, 'get_war_history', { player_tag: focusTag, seasons: 3 });
  assert.equal(focused.isError, false, JSON.stringify(focused.body));
  assert.ok(focused.body.member_weeks.length > 0);
  assert.ok(focused.body.member_weeks.every((w) => typeof w.points === 'number' && typeof w.war_days_battled === 'number'));
});
