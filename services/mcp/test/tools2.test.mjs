...
before(async () => {
  ...
  account = { accountId: acct.account_id, emailHash: acct.email_hash, isOwner: acct.is_owner, timezone: acct.timezone };
  ...
  await db.query(`insert into recording (subject_type, subject_tag, requested_by) values ('player', $1, $2)`, [
    OBSERVER,
    account.accountId,
  ]);
  const {
  ...
  await send('player_battlelog', OBSERVER, await fixture('player_battlelog/with_colosseum_duel.json'), '2026-09-03T14:30:34Z');
  await send('player', OBSERVER, await fixture('player/profile.json'), '2026-09-01T14:40:34Z');
  const day2 = structuredClone(await fixture('player/profile.json'));
  day2.trophies += 40;
  day2.battleCount += 12;
  await send('player', OBSERVER, day2, '2026-09-03T14:40:34Z');
  await send('cards', 'GLOBAL', await fixture('cards/catalog.json'), '2026-09-03T13:00:00Z');
...
test('cr_api_live: allowlist validation, then honest live_unavailable', async () => {
  ...
  const crossed = await call('cr_api_live', { path: '/clans/#J2RGCRVG/battlelog' });
