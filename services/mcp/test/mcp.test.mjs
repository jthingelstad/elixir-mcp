...
test('tools/list declares the four V1 launch tools', async () => {
  const res = await handleMcpMessage(rpc('tools/list'), context());
  assert.deepEqual(
    res.payload.result.tools.map((t) => t.name).sort(),
    ['get_coverage', 'get_player', 'list_my_players', 'query_battles'],
  );
});
