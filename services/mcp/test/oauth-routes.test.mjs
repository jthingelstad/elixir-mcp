...
function event({ method = 'POST', path: p = '/', query, body, form, ip = '9.9.9.9' }) {
  return {
    rawPath: p,
...
before(async () => {
  ...
  await db.connect();
  await db.query(`insert into account (email_hash, status) values ($1, 'approved')`, [emailHash(EMAIL)]);
  handler = makeHandler({
    databaseUrl: DB_URL,
    issuer: ISSUER,
...
test('discovery documents are well-formed and cacheable', async () => {
  const as = await handler(event({ method: 'GET', path: '/.well-known/oauth-authorization-server' }));
  ...
  const pr = await handler(event({ method: 'GET', path: '/.well-known/oauth-protected-resource' }));
...
test('full flow: register -> authorize (email, code) -> 303 with iss -> token -> live bearer', async () => {
  const reg = await handler(
    event({ path: '/oauth/register', body: JSON.stringify({ client_name: 'Claude', redirect_uris: [REDIRECT] }) }),
  );
  ...
  const start = await handler(event({ method: 'GET', path: '/oauth/authorize', query: authQuery }));
  ...
  const emailStep = await handler(
    event({ path: '/oauth/authorize', form: { step: 'email', email: EMAIL, ...authQuery } }),
  );
  ...
  const codeStep = await handler(
    event({ path: '/oauth/authorize', form: { step: 'code', email: EMAIL, code: loginCode, ...authQuery } }),
  );
  ...
  assert.equal(ctx.emailHash, emailHash(EMAIL), 'the minted bearer resolves to the account');

  // Wrong verifier on a fresh grant fails (need a new code — the old one burned).
  const bad = await handler(
    event({
      path: '/oauth/token',
      form: { grant_type: 'authorization_code', code: authCode, code_verifier: verifier, client_id, redirect_uri: REDIRECT },
    }),
  );
  assert.equal(JSON.parse(bad.body).error, 'invalid_grant', 'auth code is single-use');

  const refreshed = await handler(
    event({ path: '/oauth/token', form: { grant_type: 'refresh_token', refresh_token: tokens.refresh_token, client_id } }),
  );
...
test('unapproved emails get the identical page and no email — never an oracle', async () => {
  const reg = await handler(
    event({ path: '/oauth/register', body: JSON.stringify({ client_name: 'X', redirect_uris: [REDIRECT] }) }),
  );
  const { client_id } = JSON.parse(reg.body);
  const challenge = crypto.createHash('sha256').update('v'.repeat(43)).digest('base64url');
  const q = { client_id, redirect_uri: REDIRECT, state: '', code_challenge: challenge, code_challenge_method: 'S256', scope: '' };
  const before = sentEmails.length;
  const res = await handler(
    event({ path: '/oauth/authorize', form: { step: 'email', email: 'stranger@example.com', ...q } }),
  );
...
test('authorize rejects unregistered redirect_uri and bad challenge', async () => {
  const reg = await handler(
    event({ path: '/oauth/register', body: JSON.stringify({ client_name: 'Y', redirect_uris: [REDIRECT] }) }),
  );
  const { client_id } = JSON.parse(reg.body);
  const bad = await handler(
    event({
      ...
      query: { client_id, redirect_uri: 'https://evil.example/cb', code_challenge: 'x', code_challenge_method: 'S256' },
    }),
  );
...
test('DCR validates redirect uris and rate-limits per IP', async () => {
  const bad = await handler(
    event({ path: '/oauth/register', body: JSON.stringify({ client_name: 'Z', redirect_uris: ['http://evil.example/cb'] }) }),
  );
  assert.equal(bad.statusCode, 400);
  let limited = 0;
  for (let i = 0; i < 25; i += 1) {
    const r = await handler(
      event({ path: '/oauth/register', ip: '4.4.4.4', body: JSON.stringify({ client_name: 'Q', redirect_uris: [REDIRECT] }) }),
    );
