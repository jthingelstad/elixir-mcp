...
export const battles_deck_sets = {
  ...
  async handler(ctx, args) {
    ...
    // Every deck in the season rollup over the gates (or theirs), checked
    // against the collection in SQL: owned, forms unlocked, the lowest
    // card's level. Eight-card decks only.
    const { rows: pool } = await ctx.db.query(
      `with cand as (
       ...
       select c.deck_hash,
              ...
              bool_and(h.card_id is null or dc.form = 0 or (h.ev & dc.form) <> 0) as forms_ok,
      ...
      [roll.month, minBattles, minPlayers, tag, familiar],
    );
    const counts = {
    ...
    for (const r of pool) {
      ...
      else if (!r.forms_ok) counts.form_not_unlocked++;
      ...
      else fieldable.push(r);
    }
    counts.fieldable = fieldable.length;

    // Each fieldable deck's record by mode, and the corpus prior per mode.
    ...
    return {
      ...
      notes: notes(
        ...
        target === null
          ...
          : `Fitted to the level this player fields now (${target}): level_term is 0.5 per level a deck would sit above or below it, and a deck with a card under ${minCardLevel} is left out (candidates.below_level).`,
        counts.form_not_unlocked > 0
          ? `${counts.form_not_unlocked} candidate decks were left out only because an Evolution or Hero form in them is not unlocked; a deck's base-form version is a candidate when players played it.`
          : null,
        counts.not_owned > 0
