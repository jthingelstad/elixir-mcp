...
test("real cross-observer logs dedupe to one battle with two observations", async () => {
  ...
  const obsB =
    meta["player_battlelog/counterpart_2v2_teammate.json"].entity_key;
  ...
  assert.equal(
    battles.length,
    1,
    "both perspectives collapse to one battle row",
  );
  ...
  assert.equal(
    participants[0].n,
    4,
    "four symmetric participants, no duplicates",
  );
  ...
  for (const r of outcomes) {
    assert.equal(
      r.outcomes.length,
      1,
      `side ${r.side} teammates share one outcome`,
    );
  }
});
