...
test("profile message writes the daily snapshot; day is UTC", async () => {
  ...
  const result = await processResult(
    ...
    message({
      ...
      entityKey: tag,
      payload: profile,
      fetchedAt: "2026-09-01T23:59:00Z",
    }),
  );
...
test("later poll the same day overwrites; first sight emitted no events", async () => {
  ...
  await processResult(
    ...
    message({
      ...
      entityKey: tag,
      payload: profile,
      fetchedAt: "2026-09-01T23:59:30Z",
    }),
  );
  const { rows } = await ctx.db.query(
  ...
  assert.equal(rows[0].trophies, profile.trophies);
  const events = (
    await ctx.db.query("select count(*)::int n from player_event")
  ).rows[0].n;
...
test("a profile re-poll within the hour leaves the player row's version alone", async () => {
  ...
  const version = async () =>
    (
      await ctx.db.query(
        "select xmin::text as v from player where player_tag = $1",
        [tag],
      )
    ).rows[0].v;
  ...
  await processResult(
    ...
    message({
      ...
      entityKey: tag,
      payload: profile,
      fetchedAt: "2026-09-02T10:20:00Z",
    }),
  );
  assert.equal(await version(), before, "same identity, same hour: no write");
  await processResult(
    ctx.db,
    message({
      endpoint: "player",
...
test("battlelog ingest refreshes rollups; totals reconcile with battles", async () => {
  ...
  await processResult(
    ...
    message({
      ...
      entityKey: tag,
      payload,
      fetchedAt: "2026-09-03T14:00:34Z",
    }),
  );
  const { rows } = await ctx.db.query(
  ...
  assert.equal(
    rows[0].captured,
    truth[0].n,
    "every battle lands in exactly one rollup bucket",
  );
  assert.equal(rows[0].delta, truth[0].delta, "trophy delta reconciles");
  assert.ok(
    rows[0].decided <= rows[0].captured,
    "unresolved outcomes are not W/L/D",
  );
...
test("roster diffs emit clan events with evidence; first sight was silent", async () => {
  ...
  const events = (await ctx.db.query("select count(*)::int n from clan_event"))
    .rows[0].n;
  ...
  for (const r of rows) {
    ...
    assert.equal(
      r.window_start.toISOString(),
      "2026-09-03T14:40:34.000Z",
      "window = previous admitted poll",
    );
...
test("pre-reset window pins an extra season_roll snapshot", async () => {
  ...
  await processResult(
    ...
    message({
      ...
      entityKey: tag,
      payload: profile,
      fetchedAt: "2026-09-06T23:30:00Z",
    }),
  );
  const { rows } = await ctx.db.query(
