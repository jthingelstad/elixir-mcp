...
function admitPlayer(payload, errors) {
  ...
  if (typeof payload?.battleCount !== 'number') errors.push('battleCount:missing');
}
...
function admitBattlelog(payload, errors) {
  ...
  payload.forEach((battle, i) => {
    if (typeof battle?.battleTime !== 'string') errors.push(`[${i}].battleTime:missing`);
...
function admitRiverrace(payload, errors) {
  ...
  if (typeof payload?.periodIndex !== 'number') errors.push('periodIndex:missing');
  if (typeof payload?.sectionIndex !== 'number') errors.push('sectionIndex:missing');
  if (
...
export function admit(endpoint, payload) {
  ...
  if (!validator) return { ok: false, errors: [`endpoint:unknown:${endpoint}`] };
  const errors = [];
