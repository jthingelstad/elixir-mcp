...
test('client registration round-trips; unknown and malformed ids resolve null', async () => {
  ...
  assert.deepEqual(loaded.redirectUris, ['https://claude.ai/api/mcp/auth_callback']);
  assert.equal(await getClient(db, 'nope'), null);
  assert.equal(await getClient(db, crypto.randomBytes(18).toString('base64url')), null);
});
...
test('auth codes are single-use; the raced second redemption loses', async () => {
  ...
  const code = await createAuthCode(db, {
    ...
    scope: 'cr:read',
    codeChallenge: crypto.createHash('sha256').update('v'.repeat(43)).digest('base64url'),
  });
...
test('access tokens validate to account context; the gate applies here too', async () => {
  const { clientId } = await registerClient(db, { clientName: 'c2', redirectUris: ['https://b.example/cb'] });
  const tokens = await mintTokens(db, { clientId, accountId, scope: 'cr:read' });
  ...
  await db.query(`update account set status = 'disabled' where account_id = $1`, [accountId]);
  assert.equal(await validateAccessToken(db, tokens.accessToken), null, 'gate on every request');
  await db.query(`update account set status = 'approved' where account_id = $1`, [accountId]);
});

test('refresh rotation works and replaying the old token revokes the family', async () => {
  const { clientId } = await registerClient(db, { clientName: 'c3', redirectUris: ['https://c.example/cb'] });
  const first = await mintTokens(db, { clientId, accountId, scope: 'cr:read' });
  const rotated = await redeemRefreshToken(db, { refreshToken: first.refreshToken, clientId });
  assert.equal(rotated.status, 'ok');
  assert.ok(await validateAccessToken(db, rotated.tokens.accessToken));

  const replay = await redeemRefreshToken(db, { refreshToken: first.refreshToken, clientId });
  ...
  const again = await redeemRefreshToken(db, { refreshToken: rotated.tokens.refreshToken, clientId });
  assert.equal(again.status, 'invalid', 'revoked family refuses its newest refresh too');
});

test('client mismatch is invalid, not a revocation', async () => {
  const { clientId } = await registerClient(db, { clientName: 'c4', redirectUris: ['https://d.example/cb'] });
  const { clientId: other } = await registerClient(db, { clientName: 'c5', redirectUris: ['https://e.example/cb'] });
  const tokens = await mintTokens(db, { clientId, accountId, scope: 'cr:read' });
  const result = await redeemRefreshToken(db, { refreshToken: tokens.refreshToken, clientId: other });
  assert.equal(result.status, 'invalid');
  assert.ok(await validateAccessToken(db, tokens.accessToken), 'family untouched');
});

test('absolute family lifetime forces re-consent', async () => {
  const { clientId } = await registerClient(db, { clientName: 'c6', redirectUris: ['https://f.example/cb'] });
  const tokens = await mintTokens(db, { clientId, accountId, scope: 'cr:read' });
  await db.query(`update oauth_family set absolute_expires_at = now() - interval '1 minute' where family_id = $1`, [
    tokens.familyId,
  ]);
  const result = await redeemRefreshToken(db, { refreshToken: tokens.refreshToken, clientId });
  assert.equal(result.status, 'invalid');
  assert.equal(await validateAccessToken(db, tokens.accessToken), null, 'expired family invalidates access too');
});
